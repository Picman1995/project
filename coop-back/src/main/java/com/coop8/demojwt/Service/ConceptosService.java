...
import com.coop8.demojwt.Repository.ConceptosRepository;
...
public class ConceptosService {
    ...
    public SecuredResponse newAction(@Valid @RequestBody ConceptosRequest conceptosRequest) throws Exception {
        log.info("ConceptosService | newAction");
        log.info("__conceptosRequest: " + Util.getJsonFromObject(conceptosRequest));
        ...
        if (Util.isNullOrEmpty(conceptosRequest.getId()) || conceptosRequest.getId() == 0) {
            // INSERT
            Conceptos conceptosInsert = new Conceptos();
            conceptosInsert.setDescripcion(conceptosRequest.getDescripcion().toUpperCase());
            conceptosInsert.setFechasys(new Date());
            conceptosInsert.setUsuariosys(usuariosys);

           conceptosRepository.save(conceptosInsert);
